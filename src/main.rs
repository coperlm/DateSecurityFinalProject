...
use crypto::{
    ChameleonHash, Ed25519Impl, PqSignature,
    envelope_encrypt, generate_ed25519_keypair, generate_rsa_keypair,
};
...
struct EncryptAndMineRequest {
    /// 待加密的明文内容
    plaintext: String,
    /// 发送者 Ed25519 私钥，Base64 编码（32 字节）
...
#[derive(Serialize)]
struct KeysResponse {
    /// Ed25519 签名私钥，Base64 编码
    ed25519_private_key: String,
    /// Ed25519 验证公钥，Base64 编码
...
/// GET /keys — 生成并返回临时测试 Ed25519 密钥对（每次随机生成）。
async fn get_keys() -> impl IntoResponse {
    ...
    Json(KeysResponse {
        ed25519_private_key: B64.encode(&ed_priv),
        ed25519_public_key: B64.encode(&ed_pub),
...
async fn main() -> Result<()> {
    ...
    let (genesis_ed_priv, genesis_ed_pub) = generate_ed25519_keypair();
    let mut blockchain = Blockchain::new();
    blockchain.genesis_block(&ch, &admin_rsa_pub, &genesis_ed_priv, &genesis_ed_pub)?;
